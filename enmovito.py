...
from enmovito.gui.control_panel import ControlPanel, ShiftSelectListWidget
...
class Enmovito(QMainWindow):
    ...
    def on_plot_requested(self, selected_params, selected_display_names, x_param, x_display):
        ...
        if (current_tab_index in self.tab_figures and
            ...
            old_x_param_display = ""
            if current_tab_index in self.tab_x_params:
                old_x_param = self.tab_x_params[current_tab_index]
                for col, full_name in self.abbr_to_full.items():
                    if col == old_x_param:
                        old_x_param_display = full_name
                        break
        ...
        param_units = {}
        for i, display_name in enumerate(numeric_display_names):
            unit = self.extract_unit(display_name)
            if unit not in param_units:
                param_units[unit] = []
            param_units[unit].append((numeric_params[i], display_name))
